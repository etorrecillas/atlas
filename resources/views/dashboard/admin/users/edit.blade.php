...
@section('page_title', 'Administração | Usuários | Editar Usuário')
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header card-header-icon card-header-primary">
                    ...
                    <h4 class="card-title">Editar usuário {{ $user->email }}</h4>
                </div>
                <div class="card-body ">

                    <div class="container">
                        <div class="tab-content tab-space">
                            <form id="userUpdate" action="{{ route('admin.usuarios.update', $user) }}" method="post" novalidate="novalidate">
                                @method('PUT')
                                @csrf
                                <div class="row">
                                    ...
                                    <div class="col-sm-9 col-md-10">
                                        <div class="form-group bmd-form-group @error('name') is-focused @enderror">
                                            <input value="{{ old('name', $user->name) }}" name="name" type="text" class="form-control @error('name') error @enderror" required aria-required="true">
                                        ...
                                        <div class="dropdown bootstrap-select form-group" style="margin-top: 0px;">
                                            <select name="ranking_id" class="selectpicker" data-style="select-with-transition" title="Selecionar" data-size="7" data-live-search="true" required aria-required="true">
                                                @foreach($rankings as $ranking)
                                                    <option value="{{ $ranking->id }}" {{ old('ranking_id', $user->ranking_id) == $ranking->id ? "selected" : ""}}>{{ $ranking->short }} ({{ $ranking->title }}) </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="dropdown bootstrap-select form-group" style="margin-top: 0px;">
                                            <select name="military_branch_id" class="selectpicker" data-style="select-with-transition" title="Selecionar" data-size="7" data-live-search="true" required aria-required="true">
                                                @foreach($branches as $branch)
                                                    <option value="{{ $branch->id }}" {{ old('military_branch_id', $user->military_branch_id) == $branch->id ? "selected" : ""}}>{{ $branch->short }} ({{ $branch->title }}) </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="dropdown bootstrap-select form-group" style="margin-top: 0px;">
                                            <select name="specialty_id" class="selectpicker" data-style="select-with-transition" title="Selecionar" data-size="7" data-live-search="true" required aria-required="true">
                                                @foreach($specialties as $specialty)
                                                    <option value="{{ $specialty->id }}" {{ old('specialty_id', $user->specialty_id) == $specialty->id ? "selected" : ""}}>{{ $specialty->short }} ({{ $specialty->title }}) </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="dropdown bootstrap-select form-group" style="margin-top: 0px;">
                                            <select name="military_organization_id" class="selectpicker" data-style="select-with-transition" title="Selecionar" data-size="7" data-live-search="true" required aria-required="true">
                                                @foreach($militaryOrg as $om)
                                                    <option value="{{ $om->id }}" {{ old('military_organization_id', $user->military_organization_id) == $om->id ? "selected" : ""}}>{{ $om->short }} ({{ $om->title }}) </option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-9 col-md-10">
                                        <div class="form-group bmd-form-group @error('email') is-focused @enderror">
                                            <input value="{{ old('email', $user->email) }}" name="email" type="email" class="form-control @error('email') error @enderror" required aria-required="true" required aria-required="true">
                                    ...
                                    <div class="col-sm-9 col-md-10">
                                        <div class="dropdown bootstrap-select form-group" style="margin-top: 0px;">
                                            <select name="role_id" class="selectpicker" data-style="select-with-transition" title="Selecionar" data-size="7" required aria-required="true">
                                                @if (old('role_id', $user->roles()->first()->id) == 1)
                                ...
                                <div class="row">
                                    <label class="col-sm-3 col-md-2 col-form-label">Senha</label>
                                    <div class="col-sm-9 col-md-10">
